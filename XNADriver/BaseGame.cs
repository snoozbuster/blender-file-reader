...
namespace XNADriver
{
    ...
    public class BaseGame : Game
    {
        ...
        private void loadModelData(BlenderFile file)
        {
            ...
            PopulatedStructure obj = file.GetStructuresByAddress(file.GetStructuresByAddress(curscene["base.first"].GetValueAsUInt())[0]["object"].GetValueAsUInt())[0];
            int SDNAIndex = file.GetBlockByAddress(obj["data"].GetValueAsUInt()).SDNAIndex;
            while(file.StructureDNA.StructureList[SDNAIndex].StructureTypeName != "Mesh")
            {
                obj = file.GetStructuresByAddress(obj["id.next"].GetValueAsUInt())[0];
                SDNAIndex = file.GetBlockByAddress(obj["data"].GetValueAsUInt()).SDNAIndex;
            }
            PopulatedStructure mesh = file.GetStructuresByAddress(obj["data"].GetValueAsUInt())[0];
            ...
            if(SDNAIndex == 48) // uses loops and polys
            {
            ...
            else if(SDNAIndex == 49) // uses MFace
            {
            ...
            else // todo: there are more ways of storing meshes (SDNA index 52 is one, there may be more)
                throw new InvalidOperationException("help");
            this.vertices = new VertexPositionColor[vertices.Count];
            for(int i = 0; i < vertices.Count; i++)
                this.vertices[i] = new VertexPositionColor(vertices[i], Color.Red);
